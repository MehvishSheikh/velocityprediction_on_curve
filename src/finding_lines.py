...
def print_road_status(img, left_line, right_line):
    ...
    """ print road status (curve direction, radius of curvature, deviation, maxvelocity) """
    road_inf, curvature, deviation = road_info(left_line, right_line)
    max_velocity =calculate_max_velocity(curvature)
    
    cv2.putText(img, 'Road Status', (22,20), cv2.FONT_HERSHEY_COMPLEX, 0.7, (80, 80, 80), 2)
    # Display information on the frame
    # cv2.putText(img, f"Max Velocity: {max_velocity:.2f} m/s", (22, 33), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (80,80,80), 2)
    ...
    maxvelocity = 'Velocity : {0:0.3f}m'.format(max_velocity)
